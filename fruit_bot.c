...
#define LOW_ENERGY 5
#define SMALL_WORLD 15
#define MEDIUM_WORLD 50
#define XSMALL_MIN 2
#define SMALL_MIN 5
#define MED_MIN 10
#define BIG_MIN 15
...
int location_check(struct bot *b) {
    ...
    if (world_size < SMALL_WORLD) {
        elec_min = SMALL_MIN;
    } else if (world_size >= SMALL_WORLD && world_size < MEDIUM_WORLD) {
        elec_min = MED_MIN;
    } else {
        elec_min = BIG_MIN;
    }
    ...
    } else if ((b->battery_level <= FIND_E * elec_min) && b->turns_left > elec_min) {
...
int choose_location_type(struct bot *b) {
    ...
    if ((b->battery_level <= FIND_E * min) && b->turns_left > min && has_electricity == 1) {
...
int best_location(struct bot *b, char destination_type[MAX_NAME_CHARS], int value) {
    ...
    while (strcmp(cur_east->name, b->location->name) != 0) {
        ...
        && east_len < (FIND_F * b->maximum_move)) {
    ...
    while (strcmp(cur_west->name, b->location->name) != 0) {
        ...
        && west_len < (FIND_F * b->maximum_move)) {
...
int check_moves(struct bot *b) {
    ...
    if ((b->battery_level <= FIND_E * min) && b->turns_left > min) {
